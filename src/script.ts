...
$(document).ready(function(){
	...
	function updateTerrain(dx: number){
	...
	function playCollectionSound(){
		...
		if (state.synth){
			state.synth.triggerAttackRelease("E4", "8n", now(), 1.0);
			state.lastSoundTime=Date.now();
		}
	}
	function playReactionSound(reactionName: string){
		if (Date.now()-state.lastSoundTime<config.soundCooldown){
			return;
		}
		if (!state.synth){
		...
		switch(reactionName){
			case "Steamburst":
				...
				state.synth.triggerAttackRelease("G4", "8n", now(), 0.5);
				break;
			case "Frostburn":
				synthConfig={oscillator: { type: "triangle" as "triangle"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("A4", "8n", now(), 0.5);
				break;
			case "Sparkflare":
				synthConfig={oscillator: { type: "sawtooth" as "sawtooth"}, envelope: { attack: .01, decay: .3, sustain: 0, release: .3}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("E3", "4n", now(), 0.5);
				break;
			case "Chillshock":
				synthConfig={oscillator: { type: "square" as "square"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("F3", "8n", now(), 0.5);
				break;
			case "Voltflow":
				synthConfig={oscillator: { type: "pulse" as "pulse"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("D4", "8n", now(), 0.5);
				break;
			case "Galeweave":
				synthConfig={oscillator: { type: "sine" as "sine"}, envelope: { attack: .01, decay: .3, sustain: 0, release: .3}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("C5", "4n", now(), 0.5);
				break;
			case "Stoneguard":
				synthConfig={oscillator: { type: "triangle" as "triangle"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("B3", "8n", now(), 0.5);
				break;
			case "Wildfire":
				synthConfig={oscillator: { type: "sawtooth" as "sawtooth"}, envelope: { attack: .01, decay: .3, sustain: 0, release: .3}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("G3", "4n", now(), 0.5);
				break;
			case "Icebind":
				synthConfig={oscillator: { type: "sine" as "sine"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("A5", "8n", now(), 0.5);
				break;
			case "Lifeburst":
				synthConfig={oscillator: { type: "triangle" as "triangle"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("E4", "8n", now(), 0.5);
				break;
			case "Growthspark":
				synthConfig={oscillator: { type: "pulse" as "pulse"}, envelope: { attack: .01, decay: .2, sustain: 0, release: .2}};
				state.synth.set(synthConfig);
				state.synth.triggerAttackRelease("F4", "8n", now(), 0.5);
				break;
			case "Shockbloom":
	...
	function updateBalls(dx: number){
